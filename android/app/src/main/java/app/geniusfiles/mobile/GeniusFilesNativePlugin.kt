...
class GeniusFilesNativePlugin : Plugin() {
    ...
    // -------- Coffre-fort : chiffrement réel AES-256-GCM par blocs --------
    //
    // Format du conteneur (extension .gfenc) :
    //   "GFVE" | version(1) | tailleBloc(4, big-endian)
    //   puis, pour chaque bloc : IV(12) | longueur(4) | chiffré+tag
    //
    // Le chiffrement se fait en flux, bloc par bloc : un film de 2 Go ne
    // charge jamais plus de 1 Mo en mémoire. La clé arrive de la couche JS
    // (déchiffrée en mémoire uniquement pendant que le coffre est ouvert)
    // et n'est jamais écrite sur le disque par ce plugin.

    private val vaultTempDir: File by lazy {
        File(context.cacheDir, "gf-vault-tmp").apply { mkdirs() }
    }

    private val vaultMagic = byteArrayOf(0x47, 0x46, 0x56, 0x45) // "GFVE"
    private val vaultChunk = 1 shl 20

    private fun isAppPrivate(f: File): Boolean {
        val p = f.absolutePath
        return p.startsWith(context.cacheDir.absolutePath) ||
            p.startsWith(context.filesDir.absolutePath)
    }

    private fun vaultKeyOf(call: PluginCall): javax.crypto.spec.SecretKeySpec? {
        val b64 = call.getString("keyBase64") ?: return null
        val raw = try { Base64.decode(b64, Base64.DEFAULT) } catch (e: Throwable) { return null }
    ...
    private fun readIntBE(input: java.io.InputStream): Int {
        val b = ByteArray(4)
        if (readFullyInto(input, b, 4) != 4) return -1
    ...
    private fun readFullyInto(input: java.io.InputStream, buf: ByteArray, len: Int): Int {
        var read = 0
        while (read < len) {
    ...
    @PluginMethod
    fun vaultTempPath(call: PluginCall) {
        val name = (call.getString("name") ?: "file").replace(Regex("[^A-Za-z0-9._-]+"), "_")
        val f = File(vaultTempDir, "${System.currentTimeMillis()}-${UUID.randomUUID()}-$name")
        val out = JSObject(); out.put("path", f.absolutePath); call.resolve(out)
    }

    @PluginMethod
    fun vaultClearTemp(call: PluginCall) {
        var removed = 0
        try {
            vaultTempDir.listFiles()?.forEach { if (it.delete()) removed++ }
        } catch (e: Throwable) { /* meilleur effort */ }
        val out = JSObject(); out.put("removed", removed); call.resolve(out)
    }

    /**
     * Secret d'appareil (32 octets) conservé dans les SharedPreferences
     * privées de l'application. Il sert uniquement à réenvelopper la clé
     * maître pour le déverrouillage biométrique ; il ne suffit jamais seul
     * puisque le déverrouillage exige la validation du capteur.
     */
    @PluginMethod
    fun vaultDeviceSecret(call: PluginCall) {
        try {
            val prefs = context.getSharedPreferences("gf_vault_secure", Context.MODE_PRIVATE)
            var b64 = prefs.getString("device_secret", null)
            if (b64 == null) {
                val raw = ByteArray(32)
                java.security.SecureRandom().nextBytes(raw)
                b64 = Base64.encodeToString(raw, Base64.NO_WRAP)
                prefs.edit().putString("device_secret", b64).apply()
            }
            val out = JSObject(); out.put("secret", b64); call.resolve(out)
        } catch (e: Throwable) { call.reject("IO_FAILED", e.message) }
    }

    @PluginMethod
    fun vaultEncryptFile(call: PluginCall) {
        val source = call.getString("source") ?: return call.reject("BAD_ARGS")
        val destination = call.getString("destination") ?: return call.reject("BAD_ARGS")
        val key = vaultKeyOf(call) ?: return call.reject("BAD_KEY")
        val overwrite = call.getBoolean("overwrite", false) == true
        val src = File(source)
        val dest = File(destination)
        if (!src.exists() || src.isDirectory) return call.reject("NOT_FOUND")
        if ((!isAppPrivate(src) || !isAppPrivate(dest)) && !hasAllFilesAccess()) {
            return call.reject("DENIED", "storage permission required")
        }
        if (dest.exists() && !overwrite) return call.reject("EXISTS")
        dest.parentFile?.mkdirs()
        try {
            FileInputStream(src).use { input ->
                FileOutputStream(dest).use { output ->
                    output.write(vaultMagic)
                    output.write(1)
                    writeIntBE(output, vaultChunk)
                    val buf = ByteArray(vaultChunk)
                    while (true) {
                        val n = readFullyInto(input, buf, vaultChunk)
                        if (n <= 0) break
                        val iv = ByteArray(12)
                        ...
                        output.write(iv)
                        writeIntBE(output, ct.size)
                        output.write(ct)
                        if (n < vaultChunk) break
                    }
                    output.flush()
                    try { output.fd.sync() } catch (e: Throwable) { /* best effort */ }
                }
            }
            val out = JSObject()
            out.put("path", dest.absolutePath)
            out.put("size", dest.length())
            call.resolve(out)
        } catch (e: Throwable) {
            // Jamais de conteneur partiel : il serait indéchiffrable.
            try { dest.delete() } catch (x: Throwable) { /* ignore */ }
            call.reject("CRYPTO_FAILED", e.message)
        }
    }

    @PluginMethod
    fun vaultDecryptFile(call: PluginCall) {
        val source = call.getString("source") ?: return call.reject("BAD_ARGS")
        val destination = call.getString("destination") ?: return call.reject("BAD_ARGS")
        val key = vaultKeyOf(call) ?: return call.reject("BAD_KEY")
        val overwrite = call.getBoolean("overwrite", false) == true
        ...
        try {
            FileInputStream(src).use { input ->
                ...
                if (readFullyInto(input, header, 4) != 4 || !header.contentEquals(vaultMagic)) {
                    throw IllegalStateException("BAD_CONTAINER")
                }
                val version = input.read()
                if (version != 1) throw IllegalStateException("BAD_VERSION")
                val chunk = readIntBE(input)
                if (chunk <= 0 || chunk > (1 shl 26)) throw IllegalStateException("BAD_CONTAINER")
                FileOutputStream(dest).use { output ->
                    val iv = ByteArray(12)
                    while (true) {
                        val ivRead = readFullyInto(input, iv, 12)
                        if (ivRead == 0) break
                        if (ivRead != 12) throw IllegalStateException("TRUNCATED")
                        val len = readIntBE(input)
                        if (len <= 0 || len > chunk + 64) throw IllegalStateException("TRUNCATED")
                        val ct = ByteArray(len)
                        if (readFullyInto(input, ct, len) != len) {
                            throw IllegalStateException("TRUNCATED")
                        }
                        val cipher = javax.crypto.Cipher.getInstance("AES/GCM/NoPadding")
                        cipher.init(
                        ...
                        output.write(cipher.doFinal(ct))
                    }
                    output.flush()
                    try { output.fd.sync() } catch (e: Throwable) { /* best effort */ }
                }
    ...
    fun fileOpStart(call: PluginCall) {
        ...
        // Noms de premier niveau que l'utilisateur a explicitement autorisés
        // à remplacer : aucun écrasement n'a lieu en dehors de cette liste.
        val overwrite = ArrayList<String>()
        call.getArray("overwrite")?.let { ow ->
            for (i in 0 until ow.length()) (ow.opt(i) as? String)?.let { overwrite.add(it) }
        }
        val task = FileOpsService.start(
            context, id, mode, sources, destination, title, overwrite,
        )
        call.resolve(task.toJson())
    }

    /**
     * Décision de l'utilisateur sur un conflit signalé par le moteur natif.
     * La tâche suspendue reprend immédiatement.
     */
    @PluginMethod
    fun fileOpResolveConflict(call: PluginCall) {
        val id = call.getString("id") ?: return call.reject("INVALID", "id requis")
        val choice = call.getString("choice") ?: "skip"
        FileOpsService.resolveConflict(id, choice, call.getBoolean("applyToAll", false) == true)
        call.resolve()
    }
