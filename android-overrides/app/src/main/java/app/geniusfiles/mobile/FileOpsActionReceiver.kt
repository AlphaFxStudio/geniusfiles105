...
class FileOpsActionReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            ...
            ACTION_CONFLICT -> {
                ...
                val choice = intent.getStringExtra("choice").orEmpty()
                if (id.isNotEmpty() && choice.isNotEmpty()) {
                    FileOpsService.resolveConflict(id, choice, false)
                }
            }
            ACTION_OPEN_DEST -> {
    ...
    companion object {
        ...
        /** Décision d'un conflit depuis la notification (arrière-plan). */
        const val ACTION_CONFLICT = "app.geniusfiles.mobile.FILEOPS_CONFLICT"
