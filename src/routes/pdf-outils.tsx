...
function PdfToolsPage() {
  ...
  /* Un outil ouvert = sélection, saisie, aperçu ou opération en cours :
     aucune annonce n'est chargée ni affichée tant qu'il est ouvert. Au
     retour à la liste, la bannière peut réapparaître. */
  useEffect(() => {
    if (tool === null) return;
    suspendAds("pdf-tool");
    return () => resumeAds("pdf-tool");
  }, [tool]);
  ...
  return (
    <AppShell>
      ...
      {/* Bannière en fin de contenu, après le dernier bloc d'outils et
          avant la navigation principale : jamais superposée. */}
      <div className="mt-6">
        <InlineAdBanner slot="pdf-tools" />
      </div>
