...
import { isQuotaExceeded } from "@/lib/quota/errors";
...
import { EmptyStateAd } from "@/components/ads/EmptyStateAd";
...
function AutomationsPage() {
  ...
  return (
    <AppShell>
      ...
      {/* Quota d'exécutions du jour, avant la liste et l'état vide. */}
      <div className="mt-4">
        <QuotaBadge feature="automations" />
      </div>
                ...
                onRun={async () => {
                  try {
                    ...
                    if (isQuotaExceeded(err)) {
                      toast.warning(tr("quota.limit.automations.title"), {
                        description: tr("quota.limit.automations.desc"),
                      });
                      return;
                    }
                    toast.error(tr("automations.toast.failed.title"), {
      ...
      {/* Écran vide : le pavé publicitaire vient *après* le message et son
          bouton, dans l'espace réellement libre. Il n'est posé que si la
          place le permet, ne recouvre jamais la carte ni la navigation, et
          laisse la page intacte quand aucune annonce n'est disponible. */}
      {items.length === 0 ? <EmptyStateAd slot="empty-automations" /> : null}
      ...
      {/* Page remplie : bannière adaptative en fin de flux, après les
          automatisations et l'historique. Elle occupe son propre espace et
          disparaît proprement si aucune annonce n'est disponible. */}
      {items.length > 0 ? <InlineAdBanner slot="automations-list" /> : null}
