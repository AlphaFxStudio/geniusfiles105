...
/**
 * Miniatures des premiers éléments d'un dossier : lancées dès que la liste
 * est connue (cache ou lecture disque). Les informations textuelles ne les
 * attendent jamais — elles sont déjà peintes.
 */
function warmVisibleThumbnails(entries: FileEntry[]): void {
  const paths: string[] = [];
  ...
  if (paths.length > 0) warmThumbnails(paths, 104);
}
...
export function FilesPage() {
  ...
  /* Recherche contextuelle : elle filtre le dossier ouvert, exactement
     comme dans les catégories — jamais de bascule vers la recherche
     globale depuis le gestionnaire. */
  const [searchOpen, setSearchOpen] = useState(false);
  const [query, setQuery] = useState("");
  const pathKeyForSearch = path ? `${path.rootId}/${path.segments.join("/")}` : "";
  /* Changer de dossier repart d'une liste complète. */
  useEffect(() => {
    setSearchOpen(false);
    setQuery("");
  }, [pathKeyForSearch]);
  ...
  useEffect(() => {
    ...
    listDirectory(path).then((res) => {
      ...
      // …et les miniatures des premiers éléments, sans attendre que les
      // lignes soient montées ni bloquer l'affichage des noms/tailles.
      warmVisibleThumbnails(res.entries);
    });
    return () => {
      cancelled = true;
    };
    ...
    <AppShell>
      {path ? (
        <FilesTopBar
          ...
          onBack={goBack}
          onSearch={() => {
            /* Recherche dans le contexte courant (dossier ouvert), comme
               dans les catégories : aucune bascule vers la recherche
               globale depuis le gestionnaire. */
            setSearchOpen((v) => {
              if (v) setQuery("");
              return !v;
      ...
      {path && searchOpen && !selectionMode ? (
        <InlineSearchField
          ...
          placeholder={t("search.input.placeholderScoped", { scope: currentTitle })}
        />
      ) : null}
