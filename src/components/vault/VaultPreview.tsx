...
import { decryptToTemp, releaseTemp } from "@/lib/vault/file-crypto";
...
/**
 * Chemin lisible pour l'aperçu. Un élément chiffré est déchiffré dans le
 * cache privé de l'application le temps de la consultation seulement :
 * la copie claire est supprimée dès la fermeture de l'aperçu.
 */
function usePlainAbsolute(item: VaultItem): string | null {
  const [path, setPath] = useState<string | null>(
    item.encrypted ? null : (item.vaultAbsolutePath ?? ""),
  );
  useEffect(() => {
    if (!item.encrypted || item.isDirectory) {
      setPath(item.vaultAbsolutePath ?? "");
      return;
    }
    let cancelled = false;
    let temp: string | null = null;
    setPath(null);
    decryptToTemp(item.vaultAbsolutePath ?? "", item.name)
      .then((res) => {
        if (cancelled) {
          void releaseTemp(res);
          return;
        }
        temp = res;
        setPath(res ?? "");
    ...
    return () => {
      cancelled = true;
      void releaseTemp(temp);
    };
  }, [item.encrypted, item.isDirectory, item.name, item.vaultAbsolutePath]);
  return path;
}

function PreviewBody({ item }: { item: VaultItem }) {
  const t = useT();
  const plain = usePlainAbsolute(item);
  ...
  const url = convertFileSrc(plain);
  ...
  switch (item.kind) {
    ...
    case "text":
    case "code":
      return <TextPreview absolute={plain} />;
    default:
