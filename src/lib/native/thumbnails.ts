...
/**
 * Préchauffe les miniatures des premiers éléments d'une liste dès que son
 * contenu est connu, sans attendre que les lignes soient montées.
 *
 * La file native reste plafonnée (4 tâches) : le défilement ne subit
 * aucun ralentissement, et les vignettes déjà résolues sont ignorées.
 */
export function warmThumbnails(absolutePaths: string[], size = 320, max = 16): void {
  if (!isAndroidNative()) return;
  const px = scaleForDisplay(size);
  let n = 0;
  for (const path of absolutePaths) {
    if (n >= max) break;
    if (!canThumbnail(path)) continue;
    if (peekThumbnail(path, px)) continue;
    n++;
    void resolveThumbnail(path, px);
  }
}
