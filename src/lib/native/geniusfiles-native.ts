...
type Plugin = {
  ...
  /* Coffre-fort — chiffrement réel AES-256-GCM par blocs (flux natif).
     La clé arrive en base64 depuis la mémoire de l'application et n'est
     jamais persistée par le plugin. */
  vaultEncryptFile?: (opts: {
    source: string;
    destination: string;
    keyBase64: string;
    overwrite?: boolean;
  }) => Promise<{ path: string; size: number }>;
  vaultDecryptFile?: (opts: {
    source: string;
    destination: string;
    keyBase64: string;
    overwrite?: boolean;
  }) => Promise<{ path: string; size: number }>;
  /** Chemin de travail dans le cache privé de l'application. */
  vaultTempPath?: (opts: { name: string }) => Promise<{ path: string }>;
  /** Purge des fichiers temporaires déchiffrés. */
  vaultClearTemp?: () => Promise<{ removed: number }>;
  /** Secret d'appareil (SharedPreferences privées) pour l'enveloppe biométrique. */
  vaultDeviceSecret?: () => Promise<{ secret: string }>;
