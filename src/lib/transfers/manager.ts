...
async function begin(
  ...
  const renames = resolution.renames;
  task.renamed = renames.size;
  ...
  // Il sait désormais remplacer un élément autorisé par l'utilisateur ET
  // suspendre la tâche pour poser la question sur un doublon apparu en
  // cours de route. Sur un ancien build (sans cette capacité), un
  // remplacement repasse par le moteur JavaScript.
  // Un renommage (« conserver les deux ») décidé à l'avance passe par le
  // moteur JavaScript, seul à savoir écrire sous un autre nom.
  if (
    renames.size === 0 &&
    isNativeTransferAvailable() &&
    ...
    void startNativeTask({
      ...
      title: task.title,
      ...(overwrite.size ? { overwrite: [...overwrite] } : {}),
    }).then((ok) => {
      // Le natif a refusé (ancien build) : bascule transparente sur le JS.
      if (!ok) {
        task.native = false;
        void run(task, plannedGroups, destination, onDone, overwrite, renames);
      }
    });
    return;
  }
...
function applyNative(snap: NativeTaskSnapshot, task: Internal) {
  ...
  // Un élément ignoré n'est ni un succès ni un échec.
  task.skipped = snap.skipped ?? task.skipped;
  task.renamed = snap.renamed ?? task.renamed;
  task.succeeded = Math.max(0, snap.completed - task.failures.length - task.skipped);
...
function ensureNativeBridge() {
  ...
  onNativeTaskEvent((event, snap) => {
    ...
    publish();
    // Un doublon suspend la tâche : on pose la question dès qu'elle est
    // signalée (évènement dédié, ou état retrouvé au retour dans l'app).
    if (snap.conflict) {
      void askNativeConflict(task, snap.conflict);
...
/**
 * Le service natif a suspendu la tâche sur un doublon : on pose la même
 * question que partout ailleurs, puis on lui renvoie la décision. Une
 * seule demande par élément (le service ne redemande pas tant qu'il
 * attend), donc aucun dialogue en double.
 */
const nativeAsked = new Map<string, string>();

async function askNativeConflict(
  task: Internal,
  conflict?: { name: string; isDirectory: boolean },
) {
  if (!conflict) return;
  if (nativeAsked.get(task.id) === conflict.name) return;
  nativeAsked.set(task.id, conflict.name);
  try {
    const answer = await requestConflictDecision({
      ...
      destLabel: task.destLabel,
      remaining: 0,
      // Le moteur natif ne connaît pas le nombre de conflits restants : dès
      // que la tâche porte sur plusieurs éléments, la case « appliquer à
      // tous » doit être proposée (sinon elle n'apparaissait jamais).
      multiple: task.total > 1,
    });
    resolveNativeConflict(task.id, answer.choice, answer.applyToAll);
  } finally {
    if (nativeAsked.get(task.id) === conflict.name) nativeAsked.delete(task.id);
  }
}
...
export async function adoptNativeTransfers(): Promise<void> {
  ...
  for (const snap of await listNativeTasks()) {
    ...
    if (task) {
      applyNative(snap, task);
      if (snap.conflict) void askNativeConflict(task, snap.conflict);
    }
  }
  publish();
}
...
async function run(
  ...
  const onConflict = async (item: { name: string; isDirectory: boolean }) => {
    ...
    if (answer.applyToAll) lateBlanket = answer.choice;
  ...
  for (const group of groups) {
    ...
    const res = await transferEntries(group.parent, group.entries, destination, {
      ...
      ...(renames && renames.size ? { rename: renames } : {}),
      onProgress: (p) => {
        lastTotal = p.total;
    ...
    task.renamed = Math.max(task.renamed, res.renamed ?? 0);
...
export function summaryMessage(task: TransferTask): string {
  ...
  if (task.renamed > 0) parts.push(t("ops.transfer.renamedCount", { count: task.renamed }));
