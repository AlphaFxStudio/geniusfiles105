...
export type NativeTaskSnapshot = {
  ...
  renamed?: number;
  /** Conflit en attente de décision (tâche suspendue, rien n'est écrit). */
...
/**
 * Le moteur natif sait-il suspendre une tâche sur un doublon et attendre
 * la décision de l'utilisateur ? (faux sur un ancien build : l'appelant
 * retombe alors sur la boucle JavaScript, qui gère les conflits.)
 */
export function nativeConflictsSupported(): boolean {
  return isNativeTransferAvailable() && typeof plugin()?.fileOpResolveConflict === "function";
}

/** Transmet la décision de l'utilisateur ; la tâche reprend aussitôt. */
export function resolveNativeConflict(
  id: string,
  choice: "overwrite" | "rename" | "skip",
  applyToAll: boolean,
) {
  void plugin()?.fileOpResolveConflict?.({ id, choice, applyToAll });
}
...
export function onNativeTaskEvent(
  cb: (event: "progress" | "done" | "conflict", task: NativeTaskSnapshot) => void,
): () => void {
  const p = plugin();
  if (!p?.addListener) return () => {};
  const handles: { remove: () => void }[] = [];
  const wire = (name: string, kind: "progress" | "done" | "conflict") => {
