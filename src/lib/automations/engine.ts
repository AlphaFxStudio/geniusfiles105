...
import { commitQuota, reserveQuota } from "@/lib/quota/store";
...
/**
 * Run an automation. Never throws — sauf quand le quota quotidien
 * d'exécutions gratuites est épuisé (`QuotaExceededError`), afin que
 * l'appelant puisse proposer l'annonce récompensée.
 *
 * Une simulation (aperçu) ne consomme jamais d'utilisation.
 */
export async function runAutomation(
  ...
  /* Le débit a lieu au démarrage réel de l'exécution : une exécution
     refusée faute de quota ne consomme rien. */
  const hold = opts.simulate ? null : reserveQuota("automations");
  if (!opts.simulate && !hold) throw new QuotaExceededError("automations");
