...
import { sweepVaultTemp } from "./file-crypto";
...
export function markUnlocked(): void {
  unlocked = true;
  // Reprise de migration des éléments encore en clair (ajoutés avant le
  // chiffrement). Jamais bloquant, jamais destructif en cas d'échec.
  void import("./migrate").then((m) => m.migrateLegacyItems()).catch(() => {});
...
export function lockSession(_reason: "auto" | "background" | "manual" = "manual"): void {
  // Même si la session était déjà fermée, on s'assure qu'aucune clé ni
  // aucune copie déchiffrée ne subsiste.
  clearMasterKey();
  void sweepVaultTemp();
  if (!unlocked) return;
  unlocked = false;
