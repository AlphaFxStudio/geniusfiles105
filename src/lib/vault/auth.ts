...
import {
  ...
  isMasterKeyLoaded,
  rewrapMasterKey,
  unlockMasterKeyWithDevice,
  unlockMasterKeyWithSecret,
} from "./crypto";
...
export async function verifyBiometric(
  ...
  try {
    ...
    if (!r?.verified) return { ok: false, status, message: r?.message };
    // Le capteur ne remplace pas la clé : il autorise seulement le
    // déballage de l'enveloppe « appareil ». Sans elle (coffre-fort créé
    // avant le chiffrement), on renvoie l'utilisateur vers son code.
    if (!isMasterKeyLoaded() && hasWrappedMasterKey()) {
      const loaded = await unlockMasterKeyWithDevice();
      if (!loaded) return { ok: false, status: "failed", message: r?.message };
    }
    return { ok: true, status, message: r?.message };
  } catch (e) {
...
export async function setupVault(
  ...
  safeSet(cred);
  // Clé maître de chiffrement créée dans le même geste que le code : les
  // fichiers ajoutés ensuite sont chiffrés dès la première seconde.
  await createMasterKey(secret, biometricEnabled);
  return cred;
}
...
export async function verifySecret(secret: string): Promise<boolean> {
  ...
  clearVaultLockout();
  // Le code correct déverrouille aussi la clé maître (jamais dérivée
  // directement du code : PBKDF2 → KEK → déballage de la clé maître).
  const unlocked = await unlockMasterKeyWithSecret(secret);
  if (unlocked && cred.biometricEnabled) {
    // Enveloppe « appareil » (re)posée pour le raccourci biométrique.
    await enableDeviceUnwrap();
  }
  return true;
}
...
export async function changeSecret(
  ...
  // La clé maître est simplement réenveloppée : aucun fichier déjà
  // chiffré n'a besoin d'être retouché.
  await rewrapMasterKey(nextSecret);
  safeSet({
    ...prev,
    method,
    hash,
...
export function setBiometricEnabled(enabled: boolean): void {
  ...
  if (enabled) void enableDeviceUnwrap();
  else disableDeviceUnwrap();
}
...
export function resetCredential(): void {
  safeSet(null);
  // Sans clé maître, plus rien de l'ancien contenu n'est déchiffrable.
  forgetMasterKey();
  clearVaultLockout();
}
