...
import { ENCRYPTED_EXT, decryptFile, encryptFile, isFileCryptoAvailable } from "./file-crypto";
import {
  appendAccessLog,
  readFolders,
  readItems,
...
/**
 * Nom libre dans un dossier natif : la restauration ne doit jamais
 * écraser un fichier existant.
 */
export async function availableNativePath(dirAbs: string, name: string): Promise<string> {
  const p = nativePlugin();
  if (!p) return join(dirAbs, name);
  const dot = name.lastIndexOf(".");
  const base = dot > 0 ? name.slice(0, dot) : name;
  const suffix = dot > 0 ? name.slice(dot) : "";
  for (let i = 1; i < 500; i++) {
    const candidate = join(dirAbs, i === 1 ? name : `${base} (${i})${suffix}`);
    try {
      await p.stat({ path: candidate });
    } catch {
      return candidate;
    }
  }
  return join(dirAbs, `${base}-${Date.now()}${suffix}`);
}

/** Le chemin physique existe-t-il encore ? (garde-fou anti-perte) */
...
async function addFromPublicImpl(
  ...
  for (const src of sources) {
    ...
    if (isAndroidNative()) {
      ...
      // Fichiers : chiffrement réel AES-256-GCM avant toute suppression
      // de l'original. Dossiers (ou appareil sans pont de chiffrement) :
      // déplacement simple, comme avant.
      const encryptable = !src.isDirectory && isFileCryptoAvailable();
      const storedAbs = encryptable ? join(vaultBaseAbs(), `${id}.${ENCRYPTED_EXT}`) : vaultAbs;
      if (encryptable) {
        try {
          await encryptFile(sourceAbs, storedAbs);
          if (!(await nativeExists(storedAbs))) throw new Error("VERIFY_FAILED");
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          failed.push({ name: src.name, reason: msg });
          continue;
        }
        // L'original ne part qu'une fois le conteneur chiffré vérifié.
        try {
          await p.deletePath({ path: sourceAbs });
        } catch (err) {
          // Impossible de retirer l'original : on annule le conteneur
          // pour ne jamais laisser deux copies du même fichier.
          try {
            await p.deletePath({ path: storedAbs });
          } catch {
            /* ignore */
          }
          const msg = err instanceof Error ? err.message : String(err);
        ...
        try {
          await p.moveFile({ source: sourceAbs, destination: storedAbs, overwrite: false });
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          failed.push({ name: src.name, reason: msg });
          continue;
      ...
      upsertItem({
        ...
        vaultAbsolutePath: storedAbs,
        encrypted: encryptable,
        cipherAlgo: encryptable ? "AES-256-GCM" : undefined,
      });
...
async function restoreItemsImpl(
  ...
  for (const it of items) {
    if (isAndroidNative()) {
      ...
      const targetAbs = await availableNativePath(toAbsolutePath(target), it.name);
      const sourceAbs = it.vaultAbsolutePath ?? "";
      if (it.encrypted) {
        // Sortie du coffre-fort = déchiffrement, vérification, puis
        // seulement ensuite suppression du conteneur.
        try {
          await decryptFile(sourceAbs, targetAbs, false);
          if (!(await nativeExists(targetAbs))) throw new Error("VERIFY_FAILED");
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
        ...
        try {
          ...
          /* le fichier clair existe : l'élément sort de l'index quand même */
        }
      } else {
        try {
          await p.moveFile({
...
export function toPreviewTarget(item: VaultItem): {
  ...
  // Le nom physique est opaque (`<id>.gfenc` pour un élément chiffré).
  const stored = item.vaultAbsolutePath?.split("/").pop();
  const filename = stored ?? (item.ext ? `${item.id}.${item.ext}` : item.id);
