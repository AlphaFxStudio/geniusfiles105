...
import { commitQuota, releaseQuota, reserveQuota } from "@/lib/quota/store";
...
/**
 * Envoie un message à Genius AI.
 *
 * Consomme exactement une utilisation du quota quotidien, débitée au
 * moment de l'envoi réel : un envoi refusé (quota épuisé) ou impossible
 * (erreur immédiate) ne consomme rien. Renvoie `false` quand le quota est
 * épuisé, afin que l'écran propose l'annonce récompensée.
 */
export function sendUserMessage(text: string): boolean {
  const hold = reserveQuota("assistant");
